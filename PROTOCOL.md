...
**The list is ordered, most wakeable first**, and a device MUST order it: universally-
...
adapter at all, and otherwise the order the operating system enumerated them in. Every candidate
address stays in the list — a made-up address is REORDERED, never dropped, because a machine
record may already be keyed by one — and the cap falls after the ordering, so the eight that
travel are the eight most wakeable. The first entry is therefore the address a wake should be
...
| 2 | 2026-09-06 | **`hello.macs` names the most wakeable address first.** No new frame, no new field and no new capability: `macs` gains an ORDER a device must produce — universally-administered addresses before locally-administered ones, wired physical adapters before wireless ones and either before an interface the device cannot vouch for as a physical adapter, then the operating system's own enumeration order — with the eight-entry cap falling after the ordering rather than before it. Born from a live miss: a real MacBook was auto-added at `92:29:79:06:00:2E`, a locally-administered address of the kind macOS synthesises for bridge0, awdl0 and llw0 and rotates for a private Wi-Fi address, because that interface enumerated first; no waker on the LAN can reach a machine at such an address, and a Mac mini sitting on Ethernet was registered at one instead of at its Ethernet port. Position 0 was already load-bearing — a service creates a machine record from it (2026-09-02 below) and the wake-readiness facts diagnose against it (2026-08-30 below) — so the fix is to make position 0 mean what those two rows already assumed. Nothing is dropped: a made-up address is reordered and still sent, because a machine record may already be keyed by one, and a service MUST go on resolving a machine by ANY entry rather than by position. Additive under §10: `v` stays 2, a service that reads the list as a set is unaffected, and one that predates the rule sees a device it already understood put its best address first. |
