...
**A console client's window MUST survive the client's own failure.** On a platform where the
window belongs to the process started in it, a client that exits takes the window with it — and a
client that could not connect exits at once, so the message it printed is gone before anybody can
read it. A device MUST keep that window on the screen when the client ends badly, saying which
client it was and what its status was, until the person closes it.
