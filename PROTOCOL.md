...
**The list is ordered, most wakeable first**, and a device MUST order it: an interface whose
LINK is up — a cable in the socket, a radio associated, not merely an interface the operating
system has brought up — before one carrying no link at all; within each of those, universally-
administered addresses (the U/L bit of the first octet clear — the address an adapter's maker
burned into it) before locally-administered ones; within each of those a wired physical adapter
before a wireless one and either before an interface the device cannot vouch for as a physical
adapter at all; and otherwise the order the operating system enumerated them in. The link leads
because an adapter with nothing plugged into it cannot receive a magic packet however burned-in
its address, and a machine can keep a row of them up permanently — a Mac's Thunderbolt ports and
its anpi bridges are up, burned in and connected to nothing, six or more of them ahead of the
Wi-Fi the machine is genuinely reachable at. Every candidate address stays in the list — a
made-up address is REORDERED, never dropped, because a machine record may already be keyed by
one — and the cap falls after the ordering, so the eight that travel are the eight most
wakeable. The first entry is therefore the address a wake should be
addressed to, which is what lets a service create a machine record from it (§13, 2026-09-02) and
what the wake-readiness facts diagnose against (§13, 2026-08-30). A service MUST still resolve a
machine by ANY entry, never by position: the position says which is the best target today, not
which one an existing record was created from.
...
`host` is optional and carries the hostname of the machine the device runs **on**, at most 64
characters — a software emitter reporting what its machine calls itself; a dongle omits it, for
`macs`'s own reason: it does not run on anything. It is presentation, never identity: a service
may use it to *label* the machine (a picker offering "STUDY-PC — aa:bb:…" instead of a device
id) and as a suggested default for a machine's name, and MUST NOT use it as a matching key,
authenticate anything with it, or treat two devices reporting the same hostname as related.
Stored-not-acted like `macs` above. An empty string is not a name and MUST be treated as the
field being absent.
...
{ "t": "ssh_key_forget_result", "req_id": "…", "ok": true,
  ...
  "outcome": "forgotten", "removed": ["key", "ppk", "public", "alias"] }
```
...
half), `ppk` (the PuTTY twin beside it), `public` (the public half) and `alias` (the
`~/.ssh/config` block naming it), in the order it went — the two copies of the credential first,
because if only some of the acts can happen those are the ones that matter. **It is `[]`, never
`null`**, and it rides a REFUSAL as well as a success — a forget that removed the key and then
could not rewrite the config file has still changed the machine, and a caller that only read it on
success would show nothing about a machine that moved.
...
**`addresses`** is ordered too, and a device MUST order it: an overlay address in the CGNAT
range 100.64.0.0/10 — a mesh address, the one that reaches the machine from anywhere — before an
RFC 1918 LAN address, and within each of those the addresses of the machine's interfaces in
`hello.macs`'s own wake order, an interface with no hardware address of its own last. So the
first entry is the address worth offering first, and the first LAN address belongs to the
interface a wake is aimed at rather than to whichever one the operating system enumerated first
— which is the difference between naming a laptop's wired address and naming the wireless one
that goes away the moment its owner unplugs the dock. A device that could not read its addresses
omits the member, and a device whose interfaces are up while none holds an address yet — the
seconds after a resume, before a lease arrives — MUST omit it rather than send an empty list,
and SHOULD re-read sooner than its usual cadence rather than serving that reading until the
...
**`activeSessionProgram`** (string, agent 0.21.0) names the program moving the bytes, and is
sent beside `activeSessionKind: "ssh-transfer"` and nothing else: exactly one of `sftp` (the
sftp subsystem, which is what an sshfs mount runs on this side of the link), `scp`, `rsync` and
`git` (any of the git servers a fetch or a push runs), so a dashboard can say "rsync over SSH"
where it could only say "file transfer over SSH". It is positive-or-absent, the rule every
name in this document follows: a device sends it only for a program in the four, so a port
forward, an `exec` request running something else, and a transfer whose program the device
cannot see send no member at all — and a service MUST read the absence as "a file transfer,
program unnamed" rather than as a fifth value or an error. A device MUST omit it beside every
other kind, beside `activeSession: false` and beside an absent `activeSession`, for the reason
`activeSessionKind` is omitted there: a program named for a session the same frame says is not
running is a sentence about nothing. Where more than one transfer is active it names the one
whose activity is most recent, which is the session the kind is speaking about.
...
And only a socket the host opened SINCE THE WAKE is evidence of this sign-in. An operating system
does not tear its sockets down when it suspends, so for seconds to minutes after a resume its
tables still list the channel the host held before the sleep — a device that counts any socket
answers `signedIn: true` the instant a machine wakes, and the hand-off it unblocks opens a web
client that lists the machine offline. A device MUST therefore judge the evidence against the last
RESUME. How it tells one socket from another is its own business — ours reads the creation
timestamp the platform records where there is one, and remembers the socket's kernel identity from
the reading before the sleep where there is not. A device MAY accept a socket that predates the
resume once a bounded grace has passed, so that a nap shorter than the channel's own idle timeout
does not leave a signed-in host reported as signed out for the rest of the session; ours allows
sixty seconds. `since` is stamped from the new socket, so it dates this sign-in and not the one
before the sleep.

A device that has only just STARTED has no such instant and MUST NOT invent one. A restart is not
a wake: the machine around it has been awake all along, and a device that treated its own start
like a resume would report a host somebody signed in hours ago as signed out for the length of its
grace — which is a service being told something false about a machine nothing has happened to.
Every socket a device finds at start is ordinary evidence.
...
A resume also obliges the device to SAY the fact again, and this is the rule a service depends on
most. §4's absent-means-unknown is read by a service as "keep what you hold", so a member a frame
omits leaves the value from before the sleep standing — which after a wake is exactly the value
that is wrong. So a device MUST re-read `crdHost` after a resume and re-report it on its first
readable reading, whatever that reading says: agreement with the pre-sleep state is not silence
here, it is this machine saying the fact again about the machine it is now. The re-read MUST reach
the service on the first frame that can carry it — the `status_result` of the reconnection a
resume forces, where a device reconnects, or a `report`, or both — and a device whose reading is
not ready when that frame goes out SHOULD take one for the frame rather than send the block
without the member. A service MUST NOT treat the value it held across a machine's sleep as current
for that machine.
...
`shutdown` and `stop` describe different worlds and a device MUST NOT send one for the other.
`shutdown` says the MACHINE is going down — a shutdown and a restart are the same event here,
because from a relay's side both are the process ending and the machine leaving the network, and
the restart's own return is the next authentication. `stop` says that the emitter process
stopped and makes no claim about the machine at all: it is still up, still on the network, and
still answering everything except this. A device whose operating system will tell it which is
happening MUST send the one that is true; a device that cannot tell MUST send `stop`, which is
the reason that claims nothing. The distinction earns its MUST because a relay has nowhere else
to learn it from: an emitter that went quiet without saying its machine went with it is
indistinguishable from one that is no longer installed, and a machine shut down for the night
should not be reported as a machine that has lost its emitter.
...
carry that meaning: a service restart says `stop` and a reboot says `shutdown`, so a relay
acting on either would retire live emitters nightly. A sender MUST send it only while the
software is actually being removed, and MUST still complete the removal if the frame cannot be
delivered —
...
token was minted by an account holder, so the account exists by construction.

A `bound` answer, from either the token path or the plain email path, MAY carry one optional
field, `machine`, reporting what the service did about saving the device's host as a wakeable
machine:
...
**Four more members say what KIND of machine this is** (added 2026-09-06), additive under the
same rule and reported by any software device that can read them, `ready` capability or not:

- **`chassis`** — `laptop` or `desktop`, and nothing else. It is the firmware's own System
  Enclosure declaration (the SMBIOS type-3 chassis code) folded onto two words: 8, 9, 10, 14,
  30, 31 and 32 are `laptop`; 3–7, 13, 15, 16, 17–24 and 35 are `desktop`; every other code —
  `Other`, `Unknown`, a hand-held, a docking station, a code this vocabulary has never seen — is
  OMITTED. A device whose platform makes no such declaration reports the fact only where its own
  identifier settles it.
- **`battery`** (boolean) — the machine has a system battery. `false` is a positive finding (the
  platform was asked and said no battery); a platform that answered "unknown" omits it.
- **`standby`** — `modern`, `s3` or `none`: which sleep model the machine offers. `modern` is
  the S0 low-power idle state — Windows Modern Standby, Linux `s2idle` — in which the machine
  stays partly running and its network hardware can still answer; `s3` is the classic
  suspend-to-RAM; `none` is a machine that offers neither, which is what a virtual machine
  reports. Omitted where the platform has no such vocabulary, or the listing could not be read.
- **`wowlan`** (boolean) — the machine's FIRST WIRELESS adapter, in the `hello.macs` wake order,
  is set to wake it on a magic packet: the two halves `wakeReady` asks of the wake target, asked
  of the radio instead. Omitted where the machine has no wireless adapter (there is no adapter
  for the fact to be about), where the arming could not be read, and on a platform whose Wi-Fi
  wake is not a packet the adapter itself answers.

All four obey the block's standing rule, and it matters more here than anywhere else: **an
absent member means the device could not read the fact, and a service MUST NOT render it as
`desktop`, `false` or `none`.** Each is a positive finding or nothing — a chassis code outside
the two words, a battery flag the platform reported as "unknown status", a sleep listing the
device could not parse and a machine with no radio all send NOTHING rather than a value, because
every one of these renders as a sentence about somebody's hardware.

They exist because the alternative is a guess, and the guess is wrong in both directions. A
service with only `adapters` to go on infers "no wired adapter, therefore a laptop, therefore a
packet cannot wake it" — and a Modern Standby laptop whose radio is armed genuinely does wake on
a magic packet (Windows supports it natively in that state), while a desktop on Wi-Fi is not a
machine whose lid anybody can open. With these four a service can be deterministic where it was
inferring: offer the packet to a `standby: modern` machine with `wowlan: true`, and where it
cannot, name the wall in one true sentence — "this is a laptop on Wi-Fi; open the lid" — instead
of a sentence hedged with "most machines like this are".

Our agent reports `chassis` and `battery` on all three platforms, `standby` on Windows and Linux,
and `wowlan` on Windows and Linux. On macOS it sends neither `standby` nor `wowlan`, and that is
a decision rather than a gap: Apple's sleep is neither of this vocabulary's two words, and a
Mac's Wi-Fi wake is a sleep-proxy arrangement in which another host answers for the sleeping
machine rather than the adapter answering a packet, so neither value would be true of it.
...
|---|---|---|---|
| `install` | The `OpenSSH.Server~~~~0.0.1.0` optional feature. May need Windows Update; a refusal is `capability_refused`. | `openssh-server` (or `openssh`) through whichever package manager is on PATH — apt-get, dnf, yum, zypper, pacman, apk. None of them, and no daemon already installed, is `no_package_manager`. Where the manager cannot LOCATE the package, a device SHOULD refresh the index once and retry the install once — never a loop — because a stale index is the ordinary reason a package that exists cannot be found. Where the manager cannot reach its sources at all, the step fails with the fetch error and the wall is `no_package_source`. | Always `already`: every macOS ships sshd. |
...
**A step's `detail` is the machine's own COMPLAINT, never its first line of progress.** A real
machine answered a failed install with `Reading package lists...` — apt's opening narration,
offered to somebody as the reason their machine could not be set up. A device SHOULD take the
line that carries the package manager's own error prefix (`E:`, `Error:`, `error:`, `ERROR:`),
else the last line the command wrote to standard error, else the last line it wrote at all, else
the exit status in words; and it MUST keep the two output streams apart to do so, because a tool
that narrates on one and complains on the other loses the complaint when they are merged. One
line, never a stack trace, and bounded well inside §1's ceiling.
...
- **`sshSetupWall`** — why not, from the closed set `needs_full_disk_access`,
  `no_package_manager`, `no_package_source`, `capability_refused`, `no_privilege`.
  `no_package_source` is the opposite finding to `no_package_manager`: an installer IS there and
  what it installs from is not — an end-of-life release whose mirrors answer 404, or a machine
  with no route to them — so the next move is somebody's package sources rather than their
  distribution. It rides beside `unsupported` and
...
The three walls a device can only learn by TRYING — Windows Update refusing the capability,
macOS refusing the Remote Login switch without Full Disk Access, and a package manager unable to
reach its sources — are recorded by the device after an attempt and reported until an attempt
clears them. Each costs a network fetch or a process spawn to ask, which a status poll may not
spend. The other two are read afresh every time, and a device MUST NOT remember them: a machine
that has since gained a package manager, or whose service has been reinstalled with the rights it
needs, would otherwise report `unsupported` for a wall its owner had already cleared.
...
- **A device that can open PuTTY or WinSCP MUST write the key a second time, as `<keyfile>.ppk`
  beside it, in PuTTY's own format.** Those two clients read neither `ssh_config` nor an
  `openssh-key-v1` container, so a machine holding only the OpenSSH file holds a key they cannot
  offer — and a target whose `ssh_setup` has switched password sign-in off then refuses the
  connection with nothing on screen to explain it. The twin is DERIVED and is never a key of its
  own: same algorithm, same 32 bytes, same comment, and the same access as the key beside it,
  because it holds the same secret. It is written on the pass that creates a key AND on a pass
  that finds one already there, so a machine whose key predates a device's twin writer gains one
  without anybody minting a second key; a twin that already says what the device would write is
  left exactly as it is. Its failure is NOT the mint's — the OpenSSH half is the whole of what a
  mint promises, and `ssh_launch` writes the twin again where it needs one — so a device MUST NOT
  refuse a good key over it.
...
Asks the device to take a minted key off its OWN machine — the private half, the PuTTY twin
beside it, the public half and the `~/.ssh/config` block that names it — and answers
`ssh_key_forget_result` (§4) after the work.
...
`path`. Every key file at that path is held to it, the PuTTY twin included: the twin holds the same
private bytes, so a revoke that left it behind would leave the private half of a revoked key on the
machine in the format two clients read. This is the whole of the safety property: a privileged
service deleting inside a person's home directory deletes exactly the file the service named, or
nothing. A key of the person's own
...
**A device that holds a key for this target MUST open the client WITH it.** Without that the
launch is a bare `user@host`: OpenSSH then offers only its default identities, which a minted key
is deliberately not one of, and PuTTY offers nothing at all — so a target whose `ssh_setup` has
switched password sign-in off refuses the connection with the right key sitting on the machine the
button was pressed on. The device looks in the CONSOLE USER's own `~/.ssh/config` — read-only, the
same account the launch will run as — for a `Host` block whose `HostName` is this frame's `host`
and whose `User` is this frame's `user`, and which names an `IdentityFile`. How that key is then
handed over depends on what the client can read:

- **An OpenSSH client (`ssh`, `sftp`) is given the ALIAS and nothing else.** The block already
  carries the address, the account, the port, the identity and whatever `IdentitiesOnly` it sets,
  and `-i` would carry one of those five while silently disagreeing with the block about the rest.
  It is also the line the caller shows a person to type, so the button and the printed line do the
  same thing.
- **PuTTY and WinSCP are given the pieces and the `.ppk` twin**, because they read no config file.
  The port passed is the BLOCK's where it sets one, so the two halves of one press cannot reach
  different ports.

A device MUST NOT refuse a launch over any of this. No block, no key it can convert, a block an
earlier one would half-override — every one of them falls back to the argv it would have built
before, because the person may hold a key in an agent or sign in with a password, and a launch
that insisted on a key it could not find would refuse a connection that works.

**A command-line client's window MUST survive the client's own failure.** The window belongs to
the CLIENT on every platform: a console created for a process dies with that process, and a
terminal emulator opened to run one command closes when that command exits. A client that could
not connect exits at once, so the message it printed is gone before anybody can read it — which is
exactly the case a person needs it in. A device MUST keep that window on the screen when the
client ends badly, saying which client it was and what its status was, until the person closes
it.
...
| Version | Date | Change |
|---|---|---|
| 2 | 2026-09-12 | **A socket that predates the wake is not evidence that the host has signed in, and a resume must say the fact again.** No new frame, no new member and no new capability: two rules join `connect.crdHost` in §4, both of them about what a machine may claim in the minute after it wakes. An operating system does not tear its sockets down when it suspends, so a device that reads its socket tables at a resume still sees the signalling channel the host held BEFORE the sleep — and a device that counts it answers `signedIn: true` the instant the machine wakes, which unblocks a hand-off to a web client that lists the machine offline until somebody reloads: the very failure this member was added to prevent. So the evidence is now a socket opened SINCE THE LAST RESUME, with a bounded grace (ours sixty seconds) after which a surviving socket counts again, so a nap shorter than the channel's idle timeout does not report a signed-in host as signed out; a device that has only just started has no such instant and MUST NOT invent one, because a restart is not a wake and a device that treated it as one would report a host signed in hours ago as signed out for the length of its grace. How a device tells one socket from another is its own business; ours reads the creation timestamp where the platform records one and the socket's kernel identity where it does not. The second rule is the one a service depends on: since an absent member means "keep what you hold", a machine that wakes changed is described by its pre-sleep reading until something replaces it, so a device MUST re-read and re-report `crdHost` after a resume on its first readable reading — whatever it says, agreement included — on the first frame that can carry it, and a service MUST NOT treat a value it held across a sleep as current. The same rule reaches `wakeSource`, which is read once per wake and has no cadence to heal it: a device SHOULD let the first frame after a resume wait for that reading rather than send the block without the member, since an omitted `wakeSource` leaves a service holding the previous wake's answer until its next sample. Additive under §10: `v` stays 2, nothing on the wire changes shape, and a service that never expected the member reads its absence exactly as it always did. |
...
| 2 | 2026-09-13 | **A launched client uses the key the machine it opens on already holds, and its window survives a failure.** No new frames, no new members, no capability: three rules written into `ssh_key_mint` and `ssh_launch`, all of them about a press that opened a client and could not sign in. `ssh_launch` used to hand every client a bare `user@host`. OpenSSH then offers only its default identities — `id_rsa`, `id_ecdsa`, `id_ecdsa_sk`, `id_ed25519`, `id_ed25519_sk`, `id_dsa` — and a minted key is deliberately named none of them, so it was never offered; PuTTY reads neither `ssh_config` nor an `openssh-key-v1` container, so it had nothing to offer at all. On a target whose `ssh_setup` had switched password sign-in off, that is a refusal with the right key sitting in the profile of the person pressing the button. So a device now looks in the CONSOLE USER's own `~/.ssh/config` — read-only, the account the launch will run as — for a `Host` block naming this target and this account with an `IdentityFile`, and opens the client WITH it: an OpenSSH client gets the ALIAS alone, because the block carries the address, the account, the port, the identity and `IdentitiesOnly` where `-i` would carry one of five and disagree about the rest, and because the alias is also the line a caller prints for people to type. PuTTY and WinSCP read no config file, so they get the pieces and the key, on the BLOCK's port where it sets one so the two halves of one press cannot reach different ports. Nothing is refused over any of it — no block, no convertible key, a block an earlier one would half-override — because the person may hold a key in an agent or a password, and a launch that insisted would refuse a connection that works. The key PuTTY and WinSCP are given is the second thing written down: a device that can open either MUST write every minted key a second time as `<keyfile>.ppk` beside it, DERIVED from the same 32 bytes with the same comment and the same access, on the pass that creates a key and on a pass that finds one already there — so a machine whose key predates the writer gains one without minting a second key — and its failure is not the mint's, so a good key is never refused over it. `ssh_key_forget` takes it with the key, under the same fingerprint comparison as the other files, and `removed` gains `ppk` beside `key`, `public` and `alias`, with the two copies of the credential named first. The third rule is the window, and it is every platform's rather than one: a console created for a process dies with it and a terminal emulator opened to run one command closes when that command exits, so a client that fails takes its own message off the screen before anybody can read it, and a device MUST keep that window there with the client's name and status until the person closes it. Additive under §10: `v` stays 2, `removed` was already a closed vocabulary a caller reads rather than switches on, and a service that renders an unknown word as itself is unaffected. |
...
| 2 | 2026-09-06 | **`hello.macs` and `connect.addresses` name the most wakeable address first.** No new frame, no new field and no new capability: `macs` gains an ORDER a device must produce — an interface whose LINK is up before one carrying none, then universally-administered addresses before locally-administered ones, then wired physical adapters before wireless ones and either before an interface the device cannot vouch for as a physical adapter, then the operating system's own enumeration order — with the eight-entry cap falling after the ordering rather than before it. Born from a live miss: a real MacBook was auto-added at `92:29:79:06:00:2E`, a locally-administered address of the kind macOS synthesises for bridge0, awdl0 and llw0 and rotates for a private Wi-Fi address, because that interface enumerated first; no waker on the LAN can reach a machine at such an address, and a Mac mini sitting on Ethernet was registered at one instead of at its Ethernet port. The link is the FIRST key because administration alone moves that MacBook's problem rather than solving it: its en1-en3 and anpi0-anpi2 are up, burned in and connected to nothing, so six dead ports take the lead and the address the machine's record is keyed at can fall off the eight-entry list altogether — which a service reads as the machine having no agent while the agent is connected to it. Position 0 was already load-bearing — a service creates a machine record from it (2026-09-02 below) and the wake-readiness facts diagnose against it (2026-08-30 below) — so the fix is to make position 0 mean what those two rows already assumed. Nothing is dropped: a made-up address is reordered and still sent, because a machine record may already be keyed by one, and a service MUST go on resolving a machine by ANY entry rather than by position. **The same order settles `connect.addresses`**, whose classes (mesh before RFC 1918) were already specified and whose members inside a class were in enumeration order, so a laptop offered its wireless address where its wired one was the address to type; within each class the addresses now come in the interfaces' wake order, an interface with no hardware address last. And an address list that came back EMPTY while the machine's interfaces were up — the seconds after a resume, before a lease arrives — is omitted and re-read sooner, rather than being served as the machine's answer until the device's usual cadence comes round. Additive under §10: `v` stays 2, a service that reads either list as a set is unaffected, and one that predates the rule sees a device it already understood put its best address first. |
