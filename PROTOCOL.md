...
client that lists the machine offline. A device MUST therefore judge the evidence against an
instant: the last resume, or its own start, since a device that has just started cannot tell a
fresh channel from an old one either. How it tells one socket from another is its own business —
ours reads the creation timestamp the platform records where there is one, and remembers the
socket's kernel identity from the reading before the sleep where there is not. A device MAY accept
a socket that predates the instant once a bounded grace has passed, so that a nap shorter than the
channel's own idle timeout does not leave a signed-in host reported as signed out for the rest of
the session; ours allows sixty seconds. `since` is stamped from the new socket, so it dates this
sign-in and not the one before the sleep.
