 ...
 * firmware/test/vectors/config-v2.json byte for byte. The firmware's C tests assert the same
...
import {
  encode, decode, crc32, seqNewer, ConfigError,
  RECORD_LEN_V2, HEADER_LEN, PAYLOAD_LEN_V2, MAGIC_BYTES, FLAG,
} from '../lib/config.mjs';
import { buildUf2, parseUf2, flattenUf2, FAMILY, BLOCK_SIZE, PAYLOAD_PER_BLOCK } from '../lib/uf2.mjs';

const here = dirname(fileURLToPath(import.meta.url));
const vectors = JSON.parse(
  readFileSync(resolve(here, '../../../firmware/test/vectors/config-v2.json'), 'utf8')
);
...
test('golden vector file is self-consistent', () => {
  assert.equal(vectors.format_version, 2);
  assert.equal(vectors.record_len, RECORD_LEN_V2);
  assert.equal(vectors.header_len, HEADER_LEN);
  assert.equal(vectors.payload_len, PAYLOAD_LEN_V2);
...
test('record is exactly 475 bytes with the RWCF magic', () => {
  const r = encode({ ssid: 'x' });
  assert.equal(r.length, RECORD_LEN_V2);
...
test('rejects an owner_email longer than its field', () => {
  // 128 bytes plus the terminator is the whole field, so 128 fits and 129 cannot.
  assert.doesNotThrow(() => encode({ owner_email: 'a'.repeat(116) + '@example.com' }));
  assert.throws(() => encode({ owner_email: 'a'.repeat(117) + '@example.com' }), ConfigError);
});
...
test('buildUf2 produces well-formed blocks at the right address', () => {
  const record = encode({ ssid: 'HomeNet' });
...
test('a UF2 round-trips back to the original config', () => {
  ...
  assert.equal(back.owner_email, cfg.owner_email);
});
...
test('the sector tail is explicitly zeroed', () => {
  ...
  assert.ok(flat.data.subarray(RECORD_LEN_V2).every((b) => b === 0), 'tail must be all zero');
});
